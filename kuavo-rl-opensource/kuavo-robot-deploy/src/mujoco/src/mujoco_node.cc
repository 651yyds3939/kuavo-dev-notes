...
namespace
{
  ...
  struct SimJointCmd
  {
    ...
    bool use_rl_native_pd{false};
  };
  SimJointCmd pending_joint_cmd;
  ...
  size_t numJoints = 12;
  // RL policy / controller expect gym order (26 DoF), not MuJoCo qpos tree order (fingers sit between arms).
  constexpr std::array<const char *, 26> kRlJointNames = {
      ...
      "zarm_l1_joint", "zarm_l2_joint", "zarm_l3_joint", "zarm_l4_joint", "zarm_l5_joint", "zarm_l6_joint",
      "zarm_l7_joint", "zarm_r1_joint", "zarm_r2_joint", "zarm_r3_joint", "zarm_r4_joint", "zarm_r5_joint",
      "zarm_r6_joint", "zarm_r7_joint"};
  struct RlJointSensorMap
  {
    std::vector<int> qpos_adr;
    std::vector<int> dof_adr;
    std::vector<int> actuator_id;
    bool valid{false};
  };
  RlJointSensorMap rl_joint_map;
  // S49 has finger joints in qpos tree; head is after RL 26-DoF — must append explicitly (v46 has no fingers).
  constexpr std::array<const char *, 2> kHeadJointNames = {"zhead_1_joint", "zhead_2_joint"};
  struct HeadJointSensorMap
  {
    std::vector<int> qpos_adr;
    std::vector<int> dof_adr;
    std::vector<int> actuator_id;
  ...
  HeadJointSensorMap head_joint_map;
...
//0.99863, -0.00000, 0.05233, -0.00000, -0.01767, 0.00000, 0.77337, -0.01871, -0.00197, -0.63345, 0.88205, -0.35329, 0.01882, 0.01871, 0.00197, -0.63345, 0.88204, -0.35329, -0.01882, 
  ...
  void initRlJointSensorMap(mjModel *model)
  {
    rl_joint_map = {};
    if (model == nullptr)
    {
    ...
    for (const char *name : kRlJointNames)
    {
      const int jid = mj_name2id(model, mjOBJ_JOINT, name);
      if (jid < 0)
      {
        std::cerr << "[mujoco_node] RL joint not found in model: " << name << std::endl;
        return;
      }
      rl_joint_map.qpos_adr.push_back(model->jnt_qposadr[jid]);
      rl_joint_map.dof_adr.push_back(model->jnt_dofadr[jid]);
      const std::string act_name = std::string(name) + "_motor";
      const int aid = mj_name2id(model, mjOBJ_ACTUATOR, act_name.c_str());
      if (aid < 0)
      {
        std::cerr << "[mujoco_node] RL actuator not found: " << act_name << std::endl;
        return;
      }
      rl_joint_map.actuator_id.push_back(aid);
    }
    rl_joint_map.valid = rl_joint_map.qpos_adr.size() == kRlJointNames.size();
    if (rl_joint_map.valid)
    {
      const int zarm_r1_qpos = rl_joint_map.qpos_adr[19];
      std::cout << "[mujoco_node] RL joint sensor map ready (26 joints). "
                << "zarm_r1 qpos adr=" << zarm_r1_qpos
                << " (sequential qpos[26] would be finger — wrong for policy)" << std::endl;
    }
  }

  void initHeadJointSensorMap(mjModel *model)
  {
    head_joint_map = {};
    if (model == nullptr)
    {
    ...
    for (const char *name : kHeadJointNames)
    {
      const int jid = mj_name2id(model, mjOBJ_JOINT, name);
      if (jid < 0)
      {
      ...
      head_joint_map.qpos_adr.push_back(model->jnt_qposadr[jid]);
      head_joint_map.dof_adr.push_back(model->jnt_dofadr[jid]);
      const std::string act_name = std::string(name) + "_motor";
      const int aid = mj_name2id(model, mjOBJ_ACTUATOR, act_name.c_str());
      if (aid < 0)
      {
      ...
      head_joint_map.actuator_id.push_back(aid);
    }
    head_joint_map.valid = head_joint_map.qpos_adr.size() == kHeadJointNames.size();
    if (head_joint_map.valid)
    {
      std::cout << "[mujoco_node] Head joint sensor map ready (2 joints, appended after RL 26-DoF)"
                << std::endl;
    }
  }
  ...
  void publish_ros_data(const mjData *d, bool is_running)
  {
    ...
    const size_t n_sensor_joints = rl_joint_map.valid ? kRlJointNames.size() : numJoints;
    for (size_t i = 0; i < n_sensor_joints; i++)
    {
      if (rl_joint_map.valid)
      {
        joint_data.joint_q.push_back(d->qpos[rl_joint_map.qpos_adr[i]]);
        joint_data.joint_v.push_back(d->qvel[rl_joint_map.dof_adr[i]]);
        joint_data.joint_vd.push_back(d->qacc[rl_joint_map.dof_adr[i]]);
        joint_data.joint_current.push_back(d->actuator_force[rl_joint_map.actuator_id[i]]);
      }
      else
      {
    ...
    if (rl_joint_map.valid && head_joint_map.valid)
    {
      for (size_t i = 0; i < head_joint_map.qpos_adr.size(); ++i)
      {
        joint_data.joint_q.push_back(d->qpos[head_joint_map.qpos_adr[i]]);
        joint_data.joint_v.push_back(d->qvel[head_joint_map.dof_adr[i]]);
        joint_data.joint_vd.push_back(d->qacc[head_joint_map.dof_adr[i]]);
        joint_data.joint_current.push_back(d->actuator_force[head_joint_map.actuator_id[i]]);
      }
    }
  ...
  void applyPendingJointCmd(mjModel *model, mjData *data, const SimJointCmd &cmd)
  {
    for (int i = 0; i < model->nu; ++i)
    {
      data->ctrl[i] = 0.0;
    }
    if (cmd.use_rl_native_pd && rl_joint_map.valid)
    {
      const size_t n_rl = rl_joint_map.actuator_id.size();
      for (size_t i = 0; i < n_rl; ++i)
      {
        const int aid = rl_joint_map.actuator_id[i];
        const int qa = rl_joint_map.qpos_adr[i];
        ...
        const double ff = i < cmd.tau.size() ? cmd.tau[i] : 0.0;
        data->ctrl[aid] = ff + kp * (q_des - q) + kd * (v_des - v);
      }
      const size_t head_base = n_rl;
      if (head_joint_map.valid)
      {
        for (size_t h = 0; h < head_joint_map.actuator_id.size(); ++h)
        {
          const size_t idx = head_base + h;
          if (idx >= cmd.tau.size())
          {
            break;
          }
          const int aid = head_joint_map.actuator_id[h];
          data->ctrl[aid] = cmd.tau[idx];
        }
      }
      return;
    }
    for (size_t i = 0; i < numJoints && i < cmd.tau.size(); ++i)
    {
      if (i < static_cast<size_t>(model->nu))
      {
        data->ctrl[i] = cmd.tau[i];
      }
    }
  }
  ...
  {
    SimJointCmd cmd;
    cmd.joint_q = msg->joint_q;
    cmd.joint_v = msg->joint_v;
    cmd.tau = msg->tau;
    cmd.joint_kp = msg->joint_kp;
    cmd.joint_kd = msg->joint_kd;
    cmd.use_rl_native_pd =
        rl_joint_map.valid && msg->joint_kp.size() >= kRlJointNames.size() && msg->joint_kp[0] > 1.0;
    static bool logged_native_pd = false;
    if (cmd.use_rl_native_pd && !logged_native_pd)
    {
      logged_native_pd = true;
      ROS_INFO("[mujoco_node] RL native PD: ctrl = ff_tau + kp*(q_des-q) + kd*(v_des-v) on mapped actuators");
    }

    if (!cmd.use_rl_native_pd)
    {
    ...
    pending_joint_cmd = cmd;
    joint_tau_cmd = cmd.tau;
    cmd_updated = true;
  }
      ...
      { // todo 控制变量的生命周期
        ...
        if (user_scn) {
          ...
          if (sim.run)
          {
            ...
            // }
            updated = cmd_updated;
            SimJointCmd cmd_to_apply = pending_joint_cmd;
            cmd_updated = false;
            tau_cmd = joint_tau_cmd;
            queueMutex.unlock();
            if (updated)
            {
              applyPendingJointCmd(m, d, cmd_to_apply);
  ...
  while (ros::ok())
  {
    if (g_nh_ptr->hasParam("mujoco_init_state"))
    {
      ...
      else
      {
        ...
        qpos_init = {-0.00505, 0.00000, 0.84414, 1.00000, 0.00000, 0.00000, 0.00000,
