        ...
        if [[ "$response" =~ ^([yY][eE][sS])$ ]]; then
            ...
            if wget -O "${DOWNLOAD_PATH}" "${IMAGE_TARBALL_URL}"; then
                ...
                if sudo docker load -i "${DOWNLOAD_PATH}"; then
                    ...
                    IMAGE_NAME=$(docker images kuavo_mpc_wbc_img_gpu --format "{{.Repository}}:{{.Tag}}" | sort -V | tail -n1)
                    if [[ -z "$IMAGE_NAME" ]]; then
            ...
            echo "Okay. Please build or pull the 'kuavo_mpc_wbc_img_gpu' image manually."
