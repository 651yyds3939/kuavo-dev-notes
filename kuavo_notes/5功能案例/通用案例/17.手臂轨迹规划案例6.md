...
2. TACT 文件可以通过手部编辑软件（[使用手册](https://kuavo.lejurobot.com/beta_manual/basic_usage/kuavo-ros-control/docs/6常用工具/）导出，welcome.tact 是欢迎动作，放在同目录下的 action_files 文件夹下。

3. 用户可以自定义动作帧供贝塞尔曲线插值器使用，具体可以参考[plan_arm_traj_bezier_no_tact_demo.py](https://github.com/leju-robot/kuavo-ros-opensource/tree/master/src/demo/examples_code/hand_plan_arm_trajectory/plan_arm_traj_bezier_no_tact_demo.py)
