...
DANCE_CSV = "kuavo_action_FULLBODY_INPLACE_RAD.csv"
# 课程第一阶段：腿锁默认站姿，只练手臂编舞（adapt_dance_csv.py --profile arms_only）
DANCE_CSV_ARMS_ONLY = "kuavo_action_ARMS_ONLY_RAD.csv"
...
class RewardsCfg:
    ...
    # 姿态门控：仅站直时给模仿分，避免倒地仍拿 track 奖励（对齐 S49 / TD-MPC2）
...
class EventCfg:
    ...
    reset_robot_joints = EventTerm(
        func=local_rewards.reset_joints_to_csv_frame,
        mode="reset",
        params={
            "csv_path": DANCE_CSV,
            "position_noise": 0.02,
...
class KuavoS42PunchEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def _configure_dance_csv(self, csv_path: str, *, action_scale: float = 0.20):
        """Point obs / rewards / reset at the same dance CSV."""
        self.events.reset_robot_joints.params["csv_path"] = csv_path
        for group in (self.observations.policy, self.observations.critic):
            group.reference_joint_pos.params["csv_path"] = csv_path
            group.action_phase.params["csv_path"] = csv_path
        self.rewards.track_punch_arms.params["csv_path"] = csv_path
        self.rewards.track_punch_legs.params["csv_path"] = csv_path
        self.actions.joint_pos.scale = action_scale

    def __post_init__(self):
        super().__post_init__()
        self.scene.robot = Kuavos46_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        self._configure_dance_csv(DANCE_CSV, action_scale=0.20)


@configclass
class KuavoS42PunchArmsOnlyEnvCfg(KuavoS42PunchEnvCfg):
    """Curriculum stage 1: fixed legs, arms-only dance mimic."""

    def __post_init__(self):
        super().__post_init__()
        self._configure_dance_csv(DANCE_CSV_ARMS_ONLY, action_scale=0.20)
        # Legs stay at default in CSV; lower leg mimic weight, keep upright penalties
        self.rewards.track_punch_legs.weight = 3.0


@configclass
class KuavoS42PunchArmsOnlyEnvCfg_PLAY(KuavoS42PunchArmsOnlyEnvCfg):
    def __post_init__(self):
        super().__post_init__()
